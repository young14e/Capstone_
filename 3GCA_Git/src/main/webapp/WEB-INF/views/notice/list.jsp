	...
	<div>
		<table class="table table-striped table-bordered table-hover">
			...
			<c:forEach items="${list}" var="notice">
				<tr>
					...
					<td><a class='move' href='<c:out value="${notice.bno}"/>'>
		...
		<div>
			<div style="float: right;">
				<form class="form-inline" id='searchForm' action="/notice/list"
					method='get'>
						...
						<div class="input-group" style="margin-right: 50px;">
							<select name='type'>
								...
								<option value="TWC"
									...
									or 내용 or 작성자</option>
							</select> <input class="form-control col-xs-2" type='text' name='keyword'
								value='<c:out value="${pageMaker.cri.keyword}"/>' /> <input
								type='hidden' name='page'
								value='<c:out value="${pageMaker.cri.page}"/>' /> <input
								type='hidden' name='perPageNum'
								value='<c:out value="${pageMaker.cri.perPageNum}"/>' />
						</div>
					</div>
					<button class="btn btn-primary"
						style="float: right; margin-top: 10px; margin-left: 5px;">검색</button>
				</form>
			</div>
		</div>
					...
					function() {
						...
						$(".page-item a").on(
								...
								function(e) {
									...
									actionForm.find("input[name='page']").val(
											$(this).attr("href"));
									actionForm.submit();
								});
